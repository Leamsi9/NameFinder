const matchComposite = require ('./index.js');
...
module.exports.CompositeNames = {
    matchCompositeNames: (surnames: Array<string>, firstNames: Array<string>, titles: Array<string>, arrayToAnalyse: Array<string>, nameHolder: Array<string>, results: Array<string>) => {

        const matchAndConcatenate = new Promise(function (resolve, reject) {}); //I am using chained promises as a state machine with matches and non-matches as events to trigger state transitions

        for (let i = 0; i < surnames.length; i++) {
            for (let j = 0; j < arrayToAnalyse.length; j++) //walk through both the txtToAnalyse and the lastNames arrays from start to end
